...
$status = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT (SELECT count(replid) FROM `calonsiswa` WHERE lulus=1 AND kelamin='l') AS lulus, (SELECT count(replid) FROM `calonsiswa` WHERE lulus=2 AND kelamin='l') AS cadangan"));
