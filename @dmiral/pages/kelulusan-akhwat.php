...
$status = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT (SELECT count(replid) FROM `calonsiswa` WHERE lulus=1 AND kelamin='p') AS lulus, (SELECT count(replid) FROM `calonsiswa` WHERE lulus=2 AND kelamin='p') AS cadangan"));
...
function lulus(){
    ...
    if (r == true) {
        ...
        function(data){
          ...
          if (!obj.error) {
              window.location.href = '?pg=kelulusan-akhwat';
          } else {
